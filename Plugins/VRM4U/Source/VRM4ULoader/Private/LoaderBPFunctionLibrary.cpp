...
void ULoaderBPFunctionLibrary::VRMGenerateEpicSkeletonToHumanoidIKRig(USkeletalMesh * srcSkeletalMesh, UObject * &outRigIK, UObject * &outIKRetargeter, UObject * targetRigIK, UPoseAsset * targetPose){
	...
	}
	{
		...
		{
			FString name = FString(TEXT("IK_")) + sk->GetName();
...

		{
			...
			FString name = FString(TEXT("IK_")) + sk->GetName() + TEXT("_EpicSkeleton");
			UPackage* pkg = CreatePackage(*(SavePackagePath + "/" + name));
		...
		{
			FString name = FString(TEXT("IKR_")) + sk->GetName();
			UPackage* pkg = CreatePackage(*(SavePackagePath +"/"+ name));
		...
		// setup A-Pose
		if (Cast<UIKRigDefinition>(targetRigIK)) {
			UIKRetargeterController* c = UIKRetargeterController::GetController(ikr);
			...
			if (c->GetAsset()->GetTargetIKRig() && targetPose) {
				...
				USkeletalMesh* targetSK = d->PreviewSkeletalMesh;
